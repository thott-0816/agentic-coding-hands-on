---
description: Generate UI component specifications from Figma designs. Analyzes design elements and creates clear, stakeholder-friendly specs with validation rules and behavior descriptions.
...
2. Element metadata:
   - itemId: Figma node ID (e.g., "7648:404296")
...
- Parameters:
  ...
  - `frameId`: Figma frame ID of the target screen
  - `referenceFileKey`: (Optional) Figma file key of the reference screen
  - `referenceFrameId`: (Optional) Figma frame ID of the reference screen
- Tools (MCP / momorph):
  - `getPreferenceInstructions()`
  - `momorph/get_frame(fileKey, frameId)`
  - `momorph/get_frame_image(fileKey, frameId)`
  - `momorph/list_design_items(fileKey, frameId)`
  - `momorph/get_design_item_image(fileKey, frameId, designItemId)`
  - `runSubagent`
...
2) Fetch frame (screen-level) information
   - Call `momorph/get_frame(fileKey, frameId)` to obtain:
     - `frame.name`
     - `frame.id`
...
3) **Fetch Reference Specs (Mandatory)**
   - **Determine Reference Target:**
     - Check if `referenceFileKey` and `referenceFrameId` are provided.
     - **If provided:** Use these values directly.
     - **If NOT provided:**
       ...
       - Extract the `id` of this selected frame to use as `referenceFrameId` (and use current `fileKey` as `referenceFileKey`).
   - **Extract and Store:**
     - Call `momorph/list_design_items(referenceFileKey, referenceFrameId)`.
...
4) Fetch items and Determine Workflow
   - Call `momorph/list_design_items(fileKey, frameId)` to get the list of items and their IDs.

   **DECISION POINT: CHECK ITEM COUNT**

   **PATH A: Small Batch (<= 15 items)**
   - **Fetch Screenshot:** Call `momorph/get_frame_image(fileKey, frameId)` to get the full image of the frame.
   ...
   **CRITICAL: HOW TO CONSTRUCT SUBAGENT PROMPT**
   ...
   > 4. **IN STEP 1**:
   >    - First, **Call `momorph/get_frame_image(fileKey, frameId)`** to get the full screen context.
...
5) Write result to a markdown file (Executed by Main Agent or Subagent)
   ...
   - Create a markdown file with structure:
     ...
     Screen Info: (File Key, Frame ID, Frame Name, Output Language)
...
**Output (file)**
- Save to: `/.momorph/specs/{fileKey}/{frameId}-{screen-name}.csv`. If output file already exists, overwrite it.
